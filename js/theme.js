...
(function($) {
    ...
    $(document).on ('ready', function (){
        ...
        try {
          ...
          if(!scriptEl){
            ...
            for(var i=scripts.length-1;i>=0;i--){
              if((scripts[i].src||'').indexOf('js/theme.js')!==-1){ scriptEl=scripts[i]; break; }
            }
          }
          var base = '';
          if(scriptEl){
            var fullSrc = scriptEl.src || '';
            base = fullSrc.replace(/js\/theme\.js.*$/,'');
          }
          if(!base){ base = ''; }
          if(!document.querySelector('link[data-premium-css]')){
            var link=document.createElement('link');
            link.rel='stylesheet';
            link.href= base + 'assets/css/main.min.css?' + ver;
            link.setAttribute('data-premium-css','true');
            document.head.appendChild(link);
          }
          if(!document.querySelector('script[data-premium-js]')){
            var s=document.createElement('script');
            s.src= base + 'assets/js/main.min.js?' + ver;
            s.defer=true;
            s.setAttribute('data-premium-js','true');
        ...
        // Ensure related product images link to their product pages
        $(".related-product-slider .single-product").each(function(){
          var link = $(this).find('.product-name a').attr('href');
          if(link){
            var box = $(this).find('.image-box.related-images');
